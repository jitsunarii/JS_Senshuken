class Game {
  constructor(width, height, color){
    ...
    let elm = document.getElementById("game-screen");
    if(!elm){
  ...
  replaceScene(newScene){
    if(this.currentScene){//現在のシーンを廃棄
      this.currentScene.destroy({children: true, texture: true, baseTexture: true});
    }
    if(this.waitingScene){//waitingSceneもあれば廃棄
      this.waitingScene.destroy({children: true, texture: true, baseTexture: true});
      this.isPushedScene = false;
    }
  ...
  resizeCanvas(){
    ...
    const parent = this.app.view.parentElement;//親のサイズも変えておく
    parent.style.width = this.app.view.style.width  = `${canvasWidth}px`;
    parent.style.height = this.app.view.style.height = `${canvasHeight}px`;
  }
}
/*********************************
